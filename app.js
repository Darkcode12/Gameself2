...
document.getElementById('detail-date').addEventListener('click', () => {
  ...
  document.getElementById('inp-edit-date').value = val;
...
document.getElementById('btnSaveDate').addEventListener('click', () => {
  const g = games.find(x => x.id == detailId);
  if (!g) return;
  const val = document.getElementById('inp-edit-date').value;
  if (!val) return;
  // Parse including seconds
  g.customDate = new Date(val).getTime();
  save(); render();
  updateDetailDate(g);
